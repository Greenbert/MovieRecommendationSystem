package com.example.moviespdemo.config.recommender;
...
public class MySlopeOneRecommender {
	public List<RecommendedItem> mySlopeOneRecommender(long userID,int size){
		...
		try {
			DataModel model = new FileDataModel(new File("src/main/java/com/example/moviespdemo/config/loadData/movie_preferences.txt"));//构造数据模型
